function warlock_affliction(self)
   ...
   local uaf_casttime = select(7,GetSpellInfo("unstable affliction"))/1000
...
-- Survivability
      ...
      { "mortal coil",                 playerHP < 0.70 and jps.Moving}, -- restores 15% of total health
      ...
      { "soulshatter",                 targetThreatStatus ~= 0 and GetNumSubgroupMembers() > 1 }, -- reduces threat by 90% for all enemies within 50 yards - TODO: disable when soloing
-- Buff
...
-- Mana regain
      { "life tap",                    mana < 0.35 }, 
--      { "life tap",                jps.Moving and mana < 0.80 and playerHP > 0.7 }, 
-- Curse
...
--      { "soulburn: DS",             jps.buff("dark soul: misery") and not jps.buff("nightfall") and shards > 1 and not jps.buff("soulburn") }, 
